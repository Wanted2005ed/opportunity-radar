...
async function scholarshipPaymentById(id){const o=opportunities.find(x=>String(x.id)===String(id));if(o)return scholarshipPayment(o);toast('Scholarship opportunity could not be found.',false)}
async function scholarshipPayment(o){
  ...
  if(!m){m=document.createElement('div');m.id='scholarshipPayModal';m.className='modal hidden';m.innerHTML='<div class="detail-card scholarship-pay-modal"><button class="close" id="scholarshipPayClose">×</button><div class="eyebrow">SCHOLARSHIP ACCESS</div><h2 id="scholarshipPayTitle">Unlock this scholarship</h2><div class="quote" id="scholarshipQuote">Loading local price…</div><p class="fine">Base access is €4. The amount shown is the €4 equivalent in the supported payment currency for your profile/country. Choose your preferred secure payment provider below.</p><div class="scholarship-provider-grid"><button class="btn primary scholarship-provider" id="paystackScholarshipButton"><strong>Paystack</strong><small>Secure checkout</small></button><button class="btn scholarship-provider scholarship-provider-disabled" id="flutterwaveScholarshipButton" disabled><strong>Flutterwave</strong><small>Coming online after server setup</small></button></div><div class="pay-status" id="scholarshipPayStatus"></div></div>';document.body.appendChild(m);m.onclick=e=>{if(e.target===m)m.classList.add('hidden')};document.getElementById('scholarshipPayClose').onclick=()=>m.classList.add('hidden')}
  m.classList.remove('hidden');document.getElementById('scholarshipPayTitle').textContent=`Unlock: ${o.title}`;const qel=document.getElementById('scholarshipQuote'),status=document.getElementById('scholarshipPayStatus'),paystack=document.getElementById('paystackScholarshipButton'),flutterwave=document.getElementById('flutterwaveScholarshipButton');qel.textContent='Loading local price…';status.textContent='';paystack.disabled=true;
  ...
  const symbols={NGN:'₦',GHS:'GH₵',KES:'KSh',ZAR:'R',USD:'$',XOF:'CFA',XAF:'FCFA',RWF:'FRw',UGX:'USh',ZMW:'ZK',EUR:'€',GBP:'£'};qel.textContent=`${symbols[quote.currency]||quote.currency} ${Number(quote.amount).toLocaleString()} · €4 equivalent`;paystack.disabled=false;
  paystack.onclick=async()=>{paystack.disabled=true;flutterwave.disabled=true;status.textContent='Preparing Paystack checkout…';status.className='pay-status';const {data:init,error:initErr}=await db.functions.invoke('scholarship-access',{body:{action:'initialize',provider:'paystack',opportunityId:o.id,callbackUrl:location.href}});if(initErr||!init?.authorization_url){status.textContent=initErr?.message||initErr?.error||'Could not start Paystack payment.';status.className='pay-status error';paystack.disabled=false;flutterwave.disabled=false;return}location.href=init.authorization_url};
  flutterwave.onclick=async()=>{flutterwave.disabled=true;paystack.disabled=true;status.textContent='Preparing Flutterwave checkout…';status.className='pay-status';const {data:init,error:initErr}=await db.functions.invoke('scholarship-access',{body:{action:'initialize',provider:'flutterwave',opportunityId:o.id,callbackUrl:location.href}});if(initErr||!init?.authorization_url){status.textContent=initErr?.message||initErr?.error||'Flutterwave is not configured yet.';status.className='pay-status error';paystack.disabled=false;flutterwave.disabled=false;return}location.href=init.authorization_url};
}
async function handleScholarshipReturn(){const params=new URLSearchParams(location.search);const ref=params.get('reference')||params.get('tx_ref');const transactionId=params.get('transaction_id')||'';if(!ref)return;const {data,error}=await db.functions.invoke('scholarship-access',{body:{action:'verify',reference:ref,transaction_id:transactionId}});if(!error&&data?.unlocked){if(data.opportunity_id)scholarshipAccessCache.add(data.opportunity_id);history.replaceState({},'',location.pathname+location.hash);renderOpps();toast('Scholarship unlocked successfully.');const o=opportunities.find(x=>String(x.id)===String(data.opportunity_id));if(o)showOpportunityDetail(o)}else{history.replaceState({},'',location.pathname+location.hash);toast('Payment could not be verified yet. If you completed it, refresh shortly.',false)}}
function renderOpps(){
  ...
  $("oppList").innerHTML=list.map(o=>{
    ...
    const paid=isPaidScholarship(o); return '<article class="card opp '+(paid?'scholarship-locked':'')+'"><span class="trust">'+o.trust+'</span><span class="tag">'+o.type+'</span><h3>'+o.title+'</h3><div class="meta">'+o.meta+'</div><p style="margin-top:10px">'+o.desc+'</p>'+(paid?'<div class="scholarship-lock"><div><strong>🔒 Scholarship access</strong><small>€4 equivalent in your supported currency</small></div><button class="btn primary" onclick="scholarshipPaymentById('+o.id+')">Unlock</button></div>':'')+'<div style="display:flex;gap:8px;margin-top:15px"><button class="btn primary" onclick="openOpportunity('+o.id+')">'+(paid?'View locked details →':'View details →')+'</button><button class="btn" onclick="toggleSave('+o.id+')">'+(isSaved?'★ Saved':'☆ Save')+'</button></div></article>';
