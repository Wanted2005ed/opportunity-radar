...
async function scholarshipPayment(o){
  ...
  if(!m){m=document.createElement('div');m.id='scholarshipPayModal';m.className='modal hidden';m.innerHTML='<div class="detail-card scholarship-pay-modal"><button class="close" id="scholarshipPayClose">×</button><div class="eyebrow">SCHOLARSHIP ACCESS</div><h2 id="scholarshipPayTitle">Unlock this scholarship</h2><div class="quote" id="scholarshipQuote">Loading local price…</div><p class="fine">Base access is €4. The amount shown is the €4 equivalent in the supported payment currency for your profile/country. Payment is processed securely by Paystack or Flutterwave, depending on the available currency and provider.</p><div class="pay-status" id="scholarshipPayStatus"></div><button class="btn primary" id="scholarshipPayButton">Continue to secure payment</button></div>';document.body.appendChild(m);m.onclick=e=>{if(e.target===m)m.classList.add('hidden')};document.getElementById('scholarshipPayClose').onclick=()=>m.classList.add('hidden')}
